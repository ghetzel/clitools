...
func renameFilesIn(chanpath string) error {
	...
	log.Debugf("renaming files under directory: %v", chanpath)
	...
	if entries, err := filepath.Glob(filepath.Join(chanpath, `*.*`)); err == nil {
		for _, entry := range entries {
			...
			if mimeType := mime.TypeByExtension(filepath.Ext(entry)); mimeType != `` {
				if mediaType, _, err := mime.ParseMediaType(mimeType); err == nil {
					switch major, _ := stringutil.SplitPair(mediaType, `/`); major {
		...
		for _, base := range bases {
			log.Debugf("  base name: %v", base)

			var infoJson = filepath.Join(chanpath, fmt.Sprintf("%s.info.json", base))
